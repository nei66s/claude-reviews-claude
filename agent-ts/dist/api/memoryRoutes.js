...
router.get('/users/:userId/profile', async (req, res) => {
    const { userId } = req.params;
    try {
        const result = await query(`select * from public.user_profile where user_id = $1 limit 1`, [userId]);
...
router.get('/users/:userId/items', async (req, res) => {
    ...
    params.push(limit);
    try {
        const result = await query(`select * from public.user_memory_items 
...
function extractLikelyName(profile, items, fallback) {
...
router.get('/users/:userId/graph', async (req, res) => {
    const { userId } = req.params;
    try {
        ...
        const addNode = (n) => { if (!nodeSeen.has(n.id)) {
            nodeSeen.add(n.id);
            nodes.push(n);
        } };
        const addEdge = (s, t, k = 'contains') => {
            const id = `${s}::${t}::${k}`;
            if (!edgeSeen.has(id)) {
                edgeSeen.add(id);
                edges.push({ id, source: s, target: t, kind: k });
            }
        };
        ...
        if (profile?.summary_short)
            identityTraits.unshift(`Resumo: ${profile.summary_short}`);
        ...
        for (const item of items) {
            ...
            if (item.type === 'goal')
                addEdge(goalsNodeId, itemNodeId);
            else if (item.type === 'preference' || item.type === 'interaction_style')
                addEdge(preferencesNodeId, itemNodeId);
            else
                addEdge(identityNodeId, itemNodeId);
        }
        res.json({
...
router.patch('/users/:userId/items/:itemId', async (req, res) => {
    const { userId, itemId } = req.params;
    const { status, category, content, reason } = req.body;
    try {
        const currentResult = await query(`select status from public.user_memory_items where id = $1 and user_id = $2 limit 1`, [itemId, userId]);
        if (currentResult.rows.length === 0) {
            return res.status(404).json({ error: 'Item não encontrado.' });
        }
        const previousStatus = currentResult.rows[0].status;
        const nextStatus = status || previousStatus;
        const updateResult = await query(`update public.user_memory_items
       set status = $3,
           category = coalesce($4, category),
           content = coalesce($5, content),
           updated_at = now()
       where id = $1 and user_id = $2
       returning *`, [itemId, userId, nextStatus, category || null, content || null]);
...
router.post('/profile/compile', async (req, res) => {
    const { userId } = req.body;
    if (!userId)
        return res.status(400).json({ error: 'userId is required' });
    // Basic mock implementation for now to satisfy the frontend call
    try {
        const itemsResult = await query(`select content from public.user_memory_items where user_id = $1 and status = 'active'`, [userId]);
        const summary = `Compiled profile with ${itemsResult.rows.length} active facts.`;
        const upsertResult = await query(`insert into public.user_profile (
         user_id, summary_short, summary_long, last_compiled_at, updated_at
