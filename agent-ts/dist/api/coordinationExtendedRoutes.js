...
import { 
// Workflow History
startCoordinationWorkflow, addWorkflowSteps, assignStepToWorker, completeWorkflowStep, failWorkflowStep, getWorkflowHistory, getWorkflowSteps, getTeamWorkflowHistory, getWorkflowStats, 
// Error Handling
logWorkerError, scheduleRetry, getPendingRetries, resolveError, getTeamErrorStats, getRecentErrors, escalateError, 
// MCP
registerMCPTool, grantToolAccess, canAccessTool, logMCPToolCall, getAvailableTools, getMCPTool, disableMCPTool, getMCPToolStats, getRecentToolCalls, } from '../coordination/index.js';
const router = Router();
// ============ WORKFLOW HISTORY ENDPOINTS ============
// Start a new workflow
router.post('/team/:teamId/workflow/start', async (req, res) => {
    try {
...
router.post('/workflow/:workflowId/steps', async (req, res) => {
    try {
        const { steps } = req.body;
        if (!Array.isArray(steps) || steps.some((s) => !s.stepId || !s.text)) {
...
router.post('/workflow/:workflowId/step/:stepId/assign', async (req, res) => {
    try {
...
router.post('/workflow/:workflowId/step/:stepId/complete', async (req, res) => {
    try {
...
// Fail workflow step
router.post('/workflow/:workflowId/step/:stepId/fail', async (req, res) => {
    try {
...
// Get workflow steps
router.get('/workflow/:workflowId/steps', async (req, res) => {
    try {
...
// Get team workflows
router.get('/team/:teamId/workflows', async (req, res) => {
    try {
...
// Get workflow stats
router.get('/team/:teamId/workflow-stats', async (req, res) => {
    try {
...
// Log worker error
router.post('/team/:teamId/error/log', async (req, res) => {
    try {
...
// Schedule retry
router.post('/error/:errorId/retry', async (req, res) => {
    try {
...
// Get pending retries
router.get('/team/:teamId/pending-retries', async (req, res) => {
    try {
...
// Get error stats
router.get('/team/:teamId/error-stats', async (req, res) => {
    try {
...
// Get recent errors
router.get('/team/:teamId/recent-errors', async (req, res) => {
    try {
...
// Escalate error
router.post('/error/:errorId/escalate', async (req, res) => {
    try {
...
// Register MCP tool
router.post('/team/:teamId/mcp-tool/register', async (req, res) => {
    try {
...
// Grant tool access
router.post('/team/:teamId/mcp-tool/:toolId/access', async (req, res) => {
    try {
        const { workerAgentId, accessLevel, rateLimit } = req.body;
        if (!accessLevel) {
            return res.status(400).json({ error: 'Missing accessLevel' });
        }
        const accessId = await grantToolAccess(String(req.params.toolId), String(req.params.teamId), workerAgentId || null, accessLevel, rateLimit);
...
// Check access
router.post('/mcp-tool/:toolId/check-access', async (req, res) => {
    try {
...
router.get('/team/:teamId/mcp-tools', async (req, res) => {
    try {
...
// Get tool info
router.get('/mcp-tool/:toolId', async (req, res) => {
    try {
...
// Log tool call
router.post('/mcp-tool/:toolId/log-call', async (req, res) => {
    try {
...
// Get tool stats
router.get('/mcp-tool/:toolId/stats', async (req, res) => {
    try {
...
router.get('/mcp-tool/:toolId/calls', async (req, res) => {
    try {
        const limit = Number(req.query.limit) || 20;
...
// Disable tool
router.post('/mcp-tool/:toolId/disable', async (req, res) => {
    try {
