...
import { addTodo, clearWorkflowState, getWorkflowState, listTodos, saveWorkflowState, updateTodo, } from './store.js';
...
function normalizePermissionMode(value) {
    ...
    if (raw === 'auto')
        return 'auto';
    if (raw === 'read_only' || raw === 'readonly' || raw === 'read-only')
        return 'read_only';
    return 'ask';
}
...
function requiresExplicitUserApproval(tool) {
...
function hasExplicitApproval(tool, latestUserMessage) {
...
function assertToolPermission(tool, context) {
...
function resolveRequestedPath(targetPath, context) {
...
function toDisplayPath(targetPath, context) {
...
function resolveWorkflowChatId(context) {
...
export function getToolStatuses(context) {
    const permissionMode = normalizePermissionMode(context?.permissionMode);
    const annotate = (tool, status) => {
...
export async function runTool(tool, input, context) {
    ...
    if (tool === 'file_edit') {
        ...
        if (typeof find !== 'string' || typeof replace !== 'string')
            throw new Error('find/replace must be strings');
        const abs = assertAllowedPath(rel, context);
        ...
        if (replaced === original)
            return { ok: false, output: { changed: false } };
    ...
    if (tool === 'file_delete') {
        ...
        if (!stat.isFile())
            throw new Error('only files can be deleted with file_delete');
        await fs.unlink(abs);
    ...
    if (tool === 'file_move') {
        ...
        if (!fromPath)
            throw new Error('input.from_path required');
        if (!toPath)
            throw new Error('input.to_path required');
    ...
    if (tool === 'glob') {
        ...
        if (!pattern)
            throw new Error('input.pattern required');
    ...
    if (tool === 'grep') {
        ...
        if (!pattern)
            throw new Error('input.pattern required');
        if (!isInsideRoot(rel, context) && !context?.fullAccess)
            throw new Error('path outside project not allowed');
        const abs = assertAllowedPath(rel, context);
    ...
    if (tool === 'bash_exec' || tool === 'bash.exec') {
        if (!ALLOW_BASH_EXEC)
            throw new Error('bash.exec disabled (set ALLOW_BASH_EXEC=true to enable)');
        const cmd = input?.cmd;
        if (!cmd)
            throw new Error('input.cmd required');
        if (DANGEROUS_CMD.test(String(cmd)))
            throw new Error('Este comando e considerado perigoso e foi bloqueado por seguranca.');
...
`.trim();
    ...
    if (tool === 'web_search') {
        if (!ALLOW_WEB_FETCH)
            throw new Error('web_search disabled (set ALLOW_WEB_FETCH=true to enable)');
        const query = String(input?.query || '');
        if (!query)
            throw new Error('input.query required');
    ...
    if (tool === 'web_fetch') {
        if (!ALLOW_WEB_FETCH)
            throw new Error('web_fetch disabled (set ALLOW_WEB_FETCH=true to enable)');
    ...
    if (tool === 'workflow_replace') {
        ...
        if (!goal)
            throw new Error('input.goal required');
        if (!Array.isArray(stepsInput) || stepsInput.length === 0)
            throw new Error('input.steps must be a non-empty array');
    ...
    if (tool === 'env_get') {
        ...
        if (!Array.isArray(keys))
            throw new Error('input.keys must be array');
        const out = {};
