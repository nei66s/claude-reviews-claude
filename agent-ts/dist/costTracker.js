 ...
 */
export async function recordCost(record) {
    return withTransaction(async (client) => {
        const budgetId = `${record.userId}#${record.chatId}`;
        const now = new Date();
        const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
        // Get or create budget
        let budgetResult = await client.query(`SELECT * FROM cost_budgets WHERE id = $1 FOR UPDATE`, [budgetId]);
        if (budgetResult.rows.length === 0) {
            await client.query(`INSERT INTO cost_budgets (id, user_id, chat_id, last_reset_month, created_at, updated_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW())`, [budgetId, record.userId, record.chatId, monthStart]);
        }
        // Log cost
        await client.query(`INSERT INTO cost_logs (user_id, chat_id, model, input_tokens, output_tokens, total_cost, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [record.userId, record.chatId, record.model, record.inputTokens, record.outputTokens, record.totalCost]);
        // Update budget spent this month
        await client.query(`UPDATE cost_budgets 
       SET spent_this_month = spent_this_month + $2,
           updated_at = NOW()
       WHERE id = $1`, [budgetId, record.totalCost]);
    });
}
/**
 * Get cost breakdown for a user
 */
export async function getCostBreakdown(userId) {
    const now = new Date();
    const hourAgo = new Date(now.getTime() - 60 * 60 * 1000);
    const dayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const [totalResult, hourResult, dayResult, monthResult, byChatResult, byModelResult] = await Promise.all([
        query(`SELECT COALESCE(SUM(total_cost), 0) as total FROM cost_logs WHERE user_id = $1`, [userId]),
        query(`SELECT COALESCE(SUM(total_cost), 0) as total FROM cost_logs WHERE user_id = $1 AND created_at > $2`, [userId, hourAgo]),
        query(`SELECT COALESCE(SUM(total_cost), 0) as total FROM cost_logs WHERE user_id = $1 AND created_at > $2`, [userId, dayAgo]),
        query(`SELECT COALESCE(SUM(total_cost), 0) as total FROM cost_logs WHERE user_id = $1 AND created_at >= $2`, [userId, monthStart]),
        query(`SELECT chat_id, SUM(total_cost) as total FROM cost_logs WHERE user_id = $1 GROUP BY chat_id`, [userId]),
        query(`SELECT model, SUM(total_cost) as total FROM cost_logs WHERE user_id = $1 GROUP BY model`, [userId]),
    ]);
    const byChatId = {};
 ...
 */
export async function isMonthlyBudgetExceeded(userId, chatId) {
    const budgetId = `${userId}#${chatId}`;
    const result = await query(`SELECT spent_this_month, monthly_budget_cents FROM cost_budgets WHERE id = $1`, [budgetId]);
...
export async function cleanupOldLogs() {
    await query(`DELETE FROM cost_logs 
