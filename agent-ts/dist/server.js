...
function getPermissionMode(req) {
...
function resolveRequestedPathForFiles(req, targetPath) {
...
function parseDesktopListIntent(text) {
    const normalized = String(text || '').trim().toLowerCase();
    if (!normalized)
        return false;
    const mentionsDesktop = normalized.includes('area de trabalho') ||
        normalized.includes('área de trabalho') ||
        normalized.includes('desktop');
    const asksToList = normalized.includes('o que tem') ||
...
async function tryDirectDesktopList(messages, req) {
    const userText = getLastUserMessageText(messages);
    if (!parseDesktopListIntent(userText))
        return null;
    ...
    const lines = preview.map((entry) => `${entry.type === 'dir' ? 'Pasta' : 'Arquivo'}  ${entry.name}`);
...
function parseDirectReadIntent(text) {
    const normalized = String(text || '').trim();
    if (!normalized)
        return null;
    const lowered = normalized.toLowerCase();
    const hasReadIntent = lowered.includes('leia') ||
...
async function tryDirectDelete(messages, req) {
    const userText = getLastUserMessageText(messages);
...
async function tryDirectRead(messages, req) {
    const userText = getLastUserMessageText(messages);
...
app.post('/conversations/:id/duplicate', async (req, res) => {
    const sourceId = String(req.params.id || '').trim();
    const nextId = String(req.body?.id || '').trim();
    if (!sourceId)
        return res.status(400).json({ error: 'source conversation id required' });
    if (!nextId)
        return res.status(400).json({ error: 'new conversation id required' });
    try {
        const user = getRequestUser(req);
...
app.post('/chat', async (req, res) => {
    ...
    if (!messages || !Array.isArray(messages))
        return res.status(400).json({ error: 'messages required (array)' });
    // Simple moderation: join user messages and check
    const userText = messages.map((m) => m.content || '').join('\n');
    try {
        const mod = await moderateText(userText);
        if (!mod.allowed)
            return res.status(403).json({ error: 'content blocked by moderation', details: mod });
...
app.get('/files/raw', async (req, res) => {
    const targetPath = String(req.query?.path || '').trim();
    if (!targetPath)
        return res.status(400).json({ error: 'path required' });
    try {
