...
export async function cacheApiCall(
...
export async function getApiCacheEntry(
  conversationId: string,
  apiName: string,
  endpoint: string,
  requestParams: Record<string, any>
  ...
  const result = await query(
    `SELECT id, response_data, status_code, error_message, source_agent, cached_at
     ...
     LIMIT 1`,
    [conversationId, apiName, endpoint, JSON.stringify(requestParams)]
  )
...
export async function recordAgentResponse(
  ...
  agentRole?: string,
  responseType: 'direct' | 'follow_up' | 'correction' | 'support' | 'clarification' = 'direct',
  isPrimaryResponder: boolean = true,
  ...
  await query(
    `INSERT INTO agent_response_history (
 ...
 */
export async function getMessageResponders(conversationId: string, messageId: number) {
  const result = await query(
    `SELECT responding_agent, agent_role, response_type, is_primary_responder, 
            confidence_level, response_source_data, created_at
     FROM agent_response_history
     WHERE conversation_id = $1 AND message_id = $2
     ORDER BY is_primary_responder DESC, created_at ASC`,
 ...
 */
export async function recordAgentSupport(
  conversationId: string,
  userId: string,
  messageId: number,
  primaryAgent: string,
  supportingAgent: string,
  supportType: 'validation' | 'data_source' | 'correction' | 'clarification' | 'backup' | 'expertise',
  supportContent?: string,
  supportData?: Record<string, any>,
  feedbackType?: 'like' | 'dislike' | 'neutral' | 'improved' | 'corrected'
) {
  const id = `support-${Date.now()}-${Math.random().toString(36).substring(7)}`

  await query(
    `INSERT INTO agent_support_chain (
 ...
 */
export async function getSupportChain(conversationId: string, messageId: number) {
  const result = await query(
    `SELECT primary_agent, supporting_agent, support_type, support_content, 
 ...
 */
export async function getAgentSupportSummary(conversationId: string) {
  const result = await query(
    `SELECT primary_agent, supporting_agent, support_type, COUNT(*) as count
     FROM agent_support_chain
     WHERE conversation_id = $1
  ...
  const result = await query(
    `SELECT id, value, updated_at
     ...
     WHERE conversation_id = $1
       ...
       AND (expires_at IS NULL OR expires_at > NOW())
     LIMIT 1`,
    [conversationId, contextType, key]
  )
...
export async function getAllContextByType(
  conversationId: string,
  contextType: string
) {
  const result = await query(
 ...
 */
export async function updateAgentConversationalState(
  conversationId: string,
  userId: string,
  agentName: string,
  updates: {
    ...
    personalityState?: Record<string, any>
    expertiseContext?: Record<string, any>
    recentDecisions?: Record<string, any>
  ...
  const result = await query(
    `SELECT id FROM agent_conversational_state 
  ...
  if (result.rows.length === 0) {
    ...
    await query(
      `INSERT INTO agent_conversational_state (
    ...
    // Update existing
    await query(
      `UPDATE agent_conversational_state SET
...
export async function getAgentConversationalState(conversationId: string, agentName: string) {
  const result = await query(
    `SELECT id, agent_role, agent_emoji, is_active, last_message_index, personality_state,
...
export async function getAllAgentConversationalStates(conversationId: string) {
  const result = await query(
    `SELECT agent_name, agent_role, agent_emoji, is_active, last_message_index, personality_state,
            expertise_context, recent_decisions, confidence_score, mood_indicator,
            support_provided_count, corrections_made, updated_at
     FROM agent_conversational_state
     WHERE conversation_id = $1
...
export async function recordAgentInteraction(
  ...
  fromAgent: string,
  toAgent: string,
  interactionType: 'supports' | 'corrects' | 'validates' | 'questions' | 'builds_on' | 'disagrees' | 'clarifies' | 'provides_data',
  lastInteractionMessageIndex?: number,
  contextOfInteraction?: string
) {
  // Check if interaction exists
  const result = await query(
    `SELECT id FROM agent_interaction_graph
     WHERE conversation_id = $1 AND from_agent = $2 AND to_agent = $3 AND interaction_type = $4
     LIMIT 1`,
    [conversationId, fromAgent, toAgent, interactionType]
  )
  ...
  if (result.rows.length === 0) {
    ...
    await query(
      `INSERT INTO agent_interaction_graph (
    ...
    // Increment count
    await query(
      `UPDATE agent_interaction_graph SET
...
export async function getAgentInteractions(conversationId: string, filterAgent?: string) {
...
export async function storeMessageTrace(
  messageId: number,
  conversationId: string,
  userId: string,
  respondingAgent?: string,
...
export async function getMessageTrace(messageId: number) {
  const result = await query(
    `SELECT id, responding_agent, supporting_agents, tool_calls, agent_reasoning, 
...
export async function updateConversationMetadata(
  ...
  updates: {
    ...
    agentParticipationCount?: Record<string, number>
    agentSupportMap?: Record<string, any>
...
export async function getConversationMetadata(conversationId: string) {
  const result = await query(
    `SELECT last_accessed_agent, active_agents, agent_sequence, agent_participation_count, agent_support_map,
 ...
 */
export async function loadFullConversationContext(conversationId: string) {
  return {
 ...
 */
export async function getConversationDynamicsSummary(conversationId: string) {
...
export async function cleanupConversationContext(conversationId: string) {
