...
export type PersistedAgentProfile = {
  agentId: string
  profile: unknown
  createdAt: string
  updatedAt: string
...
export async function upsertAgentProfile(agentId: string, profile: unknown) {
  const normalized = String(agentId || '').trim()
  if (!normalized) throw new Error('agentId required')

  await query(
    `INSERT INTO coordination_agent_profiles (agent_id, profile)
     VALUES ($1, $2)
...
export async function getAgentProfile(agentId: string): Promise<PersistedAgentProfile | null> {
  const normalized = String(agentId || '').trim()
  if (!normalized) return null

  const result = await query(
    `SELECT agent_id, profile, created_at, updated_at
...
export async function listAgentProfiles(prefix?: string) {
