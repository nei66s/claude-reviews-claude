...
async function log(title: string, data: unknown) {
  console.log(`\n${'='.repeat(60)}`)
  console.log(`📍 ${title}`)
...
async function test() {
  try {
    ...
    const initData = (await initRes.json()) as { success: boolean; team: unknown; error?: string }
    if (!initData.success) throw new Error(`Failed to init family team: ${initData.error || 'Unknown error'}`)
    ...
    const membersData = (await membersRes.json()) as { members: unknown[]; count: number }
    ...
    const templatesData = (await templatesRes.json()) as { templates: unknown[]; count: number }
    ...
    const workflowData = (await workflowRes.json()) as { success: boolean; workflow: Workflow; error?: string }
    if (!workflowData.success) throw new Error(`Workflow creation failed: ${workflowData.error || 'Unknown error'}`)
    ...
    const customData = (await customRes.json()) as { success: boolean; workflow: Workflow }
    if (!customData.success) throw new Error('Custom workflow creation failed')
      ...
      messages: [
        {
          ...
          to: 'Betinha',
          message:
            'Betinha! Temos uma oportunidade de lançar Nova feature. Você acha viável do ponto de vista financeiro?',
        },
        {
          from: 'Betinha',
          to: 'Pimpim',
          message:
            'Oi Pimpim! 💚 Analisei e é viável. Precisamos de 15% do orçamento Q2. Posso delegar para Chubaka testar qualidade depois que Kitty terminar o design visual?',
        },
        {
          from: 'Pimpim',
          to: 'Bento',
          message:
            'Bento, qual sua análise de risco? Temos um timeline apertado...',
        },
        {
          from: 'Bento',
          to: 'Pimpim',
          message:
            'Timeline está apertado mas viável. Minha preocupação: faltam testes de integração. Preciso de mais 2 dias para validar.',
        },
        {
          from: 'Pimpim',
          to: 'Betinha + Bento',
          message:
            'OK! Vamos dar os 2 dias extras para Bento. Betinha, você consegue ajustar a data de lançamento?',
        },
        {
          from: 'Betinha',
          to: 'Pimpim',
          message:
            'Conseguimos! Bóra fazer isso direito. Aviso marketing para ajustar timeline.',
        },
      ],
    }
