...
import type { StoredMessage, StoredConversation } from './store'
...
export type ContextCompression = {
  originalMessages: StoredMessage[]
  compressedMessage: StoredMessage
 ...
 */
function summarizeMessages(messages: StoredMessage[]): string {
 ...
 */
export function compressMessages(messages: StoredMessage[]): ContextCompression | null {
...
export async function assembleContext(
  ...
  // Build output
  const outputMessages: Array<{ role: string; content: string }> = []
...
export async function buildOptimizedContext(
  ...
  const result = await query(
    `SELECT m.role, m.content, m.trace_json 
     FROM messages m 
