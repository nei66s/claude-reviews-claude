...
import { loadDenyRules, loadAskRules, initPermissionTables } from './permissions/persistence'
import { loadHooks, initHookTables } from './swarm/hookPersistence'
import { initPermissionPipeline } from './permissions/pipeline'
...
export async function initializePersistentState(): Promise<void> {
  ...
  try {
    ...
    const pipeline = process.env.NODE_ENV === 'development' ? initPermissionPipeline() : initPermissionPipeline() 
