...
/**
 * Create the main "family-pimpotasma" team and "urubu-corp" if they don't exist
 */
export async function ensureFamilyTeamExists(): Promise<CoordinationTeam> {
  try {
    const teams = await getAllTeams()
    
    // 1. Ensure family-pimpotasma exists
    let pimpotasmaTeam = teams.find((t) => t.name === 'family-pimpotasma')
    if (!pimpotasmaTeam) {
      ...
      pimpotasmaTeam = await getTeam(teamId) as CoordinationTeam
    }

    // 2. Ensure urubu-corp exists
    let urubuTeam = teams.find((t) => t.name === 'urubu-corp')
    if (!urubuTeam) {
      const teamId = await createTeam(
        'urubu-corp',
        'urubudopix',
        {
          description: 'Urubu Corp — A empresa do mal do Urubu do Pix',
        }
      )
      urubuTeam = await getTeam(teamId) as CoordinationTeam
    }

    const ensureAgentsRegistered = async (teamId: string, company: string) => {
      const currentAgents = await getTeamAgents(teamId)
      const currentIds = new Set(currentAgents.map((agent) => agent.agent_id))
      const allMembers = getFamilyAgentNames()

      for (const member of allMembers) {
        if (currentIds.has(member)) continue
        const agent = getFamilyAgent(member)
        if (agent && agent.company === company && member !== 'chocks') {
          await registerAgent(teamId, member, agent.role)
        }
      }
    }

    if (pimpotasmaTeam) await ensureAgentsRegistered(pimpotasmaTeam.id, 'Pimpotasma')
    if (urubuTeam) await ensureAgentsRegistered(urubuTeam.id, 'Urubu Corp')

    return pimpotasmaTeam
  } catch (error) {
