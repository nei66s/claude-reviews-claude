...
import { createTeam, registerAgent, sendMessage, getTeamAgents, updateAgentStatus } from './mailbox'
import type { CoordinationTeam, CoordinationAgent } from './mailbox'

export interface WorkerSpec {
  role: string // 'researcher', 'implementer', 'tester', etc
 ...
 */
function buildWorkerPrompt(spec: WorkerSpec, workerId: string, coordinatorId: string): string {
  const lines = [
 ...
 */
export async function createCoordinationTeam(
  leaderAgentId: string,
  teamName: string
): Promise<{ teamId: string; config: CoordinationTeam }> {
...
): Promise<{ status: 'completed' | 'blocked' | 'error'; result?: string }> {
 ...
 */
export async function getTeamWorkers(teamId: string): Promise<Array<CoordinationAgent & { statusReadable: string }>> {
