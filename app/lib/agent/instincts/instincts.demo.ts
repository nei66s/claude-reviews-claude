...
import { instinctEngine } from './instinct-engine'
import { checkReflexes } from './reflexes'
import { cacheHandler } from './cache-handler'
import { circuitBreakerManager } from './circuit-breaker'
import { habitManager } from './habit-patterns'
import { socialProtocolManager } from './social-protocols'
...
async function demoFullOrchestration() {
  ...
  for (const scenario of scenarios) {
    ...
    const response = await instinctEngine.process(scenario as any)
...
async function demoMonitoring() {
  ...
  console.log('\nCircuit Breakers:')
  Object.entries(status.circuitBreakers).forEach(([id, state]: any) => {
  ...
  status.habits.topHabits.forEach((h: any) => {
