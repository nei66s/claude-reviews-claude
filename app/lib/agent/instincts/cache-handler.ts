...
export class CacheHandler {
  ...
  private readonly minConfidence: number = 0.85

  /**
   * Compute similarity hash for query (simplified: just remove noise)
   */
  private computeHash(query: string): string {
   ...
   * Returns 0-1 score
   */
  private computeSimilarity(query1: string, query2: string): number {
   ...
   */
  store(query: string, response: string, confidence: number = 1.0): void {
   ...
   */
  getStats(): { size: number; avgHits: number; topHits: string[] } {
