...
export type Reflex = {
  ...
  action: 'block' | 'alert' | 'throttle' | 'deny'
 ...
 */
export const DANGER_REFLEXES: Reflex[] = [
  {
 ...
 */
export const WARNING_REFLEXES: Reflex[] = [
  {
...
export function checkReflexes(
  ...
  for (const reflex of DANGER_REFLEXES) {
    const matches =
      typeof reflex.pattern === 'function' ? reflex.pattern(input) : reflex.pattern.test(input)
 ...
 */
export function describeReflexes(): string {
