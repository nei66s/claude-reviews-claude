...
import { circuitBreakerManager } from './instincts/circuit-breaker'
 ...
 */
export async function getBudget(userId: string, chatId: string): Promise<TokenBudget> {
  return withTransaction(async (client) => {
    ...
    const result = await client.query(
      `SELECT * FROM token_budgets WHERE id = $1 FOR UPDATE`,
      [id],
    )

    if (result.rows.length === 0) {
      await client.query(
        `INSERT INTO token_budgets (id, user_id, chat_id, last_reset_hour, last_reset_day, updated_at, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`,
        [id, userId, chatId, hour, day],
      )
      return {
        userId,
        chatId,
 ...
 * Record token usage
 */
export async function recordUsage(userId: string, chatId: string, tokens: number) {
  return withTransaction(async (client) => {
    const id = `${userId}#${chatId}`
    ...
    await client.query(
      `UPDATE token_budgets 
       SET tokens_used_this_hour = $2,
           ...
           updated_at = NOW()
       WHERE id = $1`,
      [id, newHourly, newDaily, newMonthly],
    )

    // Log usage
    await client.query(
      `INSERT INTO token_usage (user_id, chat_id, tokens_used, used_at)
       VALUES ($1, $2, $3, NOW())`,
      [userId, chatId, tokens],
    )
 ...
 */
export async function getStats(userId: string, chatId: string): Promise<TokenStats> {
 ...
 */
export async function isWithinLimits(userId: string, chatId: string, estimatedTokens: number) {
