...
import { runAgent, streamAgent } from './llm'
import { query, closePool, initDatabase } from './db'
import { instinctEngine } from './instincts/instinct-engine'
import { cacheHandler } from './instincts/cache-handler'
import { emotionalStateManager } from './instincts/emotions'
import { detectSuccess, detectError, detectEngagement, processEmotionEvent } from './instincts/emotion-triggers'
import { buildEmotionalHealthSummary } from './instincts/emotion-responses'
import {
  createConversation,
  deleteConversation,
...
import { getToolStatuses, runTool, type PermissionMode } from './tools'
import { moderateText } from './moderation'
import { initPermissionPipeline, getPermissionPipeline } from './permissions/index'
import { initDefaultRules } from './permissions/defaults'
import { initHooks } from './hooks/index'
import { AuditLogger } from './audit/logger'
import { initializePersistentState } from './initialization'
import { startBackgroundJobs, stopBackgroundJobs } from './jobs/backgroundJobs'
import permissionsRoutes from './api/permissionsRoutes'
import auditRoutes from './api/auditRoutes'
import coordinationRoutes from './api/coordinationRoutes'
import coordinationExtendedRoutes from './api/coordinationExtendedRoutes'
import queryEngineRoutes from './api/queryEngineRoutes'
import agentPersonalityRoutes from './api/agentPersonalityRoutes'
import memoryRoutes from './api/memoryRoutes'
import { recordUsage } from './tokenManager'
...
function parseDirectTerminalIntent(text: string): string | null {
...
async function tryDirectTerminal(messages: { role: string; content: string | unknown }[], req: Request) {
  ...
  let executionResult: { stdout?: string; stderr?: string } = {}
  ...
  try {
    ...
    if (result?.output) {
      executionResult = result.output as { stdout?: string; stderr?: string }
    }
  } catch (err) {
...
app.post('/chat', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (chatId) {
      ...
      assistantMessageId = (savedChat?.messages?.[savedChat.messages.length - 1] as any)?.id
    }
